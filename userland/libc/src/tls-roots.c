/* tls-roots.c - the root certificates, compiled into the binary.
 *
 * Mozilla's CA list, taken from the curl.se packaging of it:
 *
 *   https://curl.se/ca/cacert.pem      retrieved 2026-09-09
 ...
 *
 * 121 anchors, in the shape BearSSL wants: the issuer name and the
 * public key already pulled out of each certificate, so nothing has to
 * decode PEM or walk X.509 at startup.
 *
 * They are compiled in rather than read from a file the way net.c
 * reads /data/ssl/cert.pem. A file on the data partition is one more
 * thing that can be missing, truncated, or quietly replaced, and a
 * replaced root list is the failure that looks exactly like success:
 * every certificate still verifies, just against somebody else's CA.
 * With the list in .rodata next to the code that uses it there is
 * nothing to look for at run time and nothing to swap out without
 * rebuilding the image.
 *
 * The cost is about 64 KB - roughly 57 KB of names and keys, plus a
 * 7.6 KB table of pointers into them. The other cost is that the list
 * is only as fresh as the last build, so regenerate it when a root is
 * added or withdrawn. brssl is BearSSL 0.6's own tool, built by
 * running make in an unpacked BearSSL tree:
 *
 *   curl -sS -o cacert.pem https://curl.se/ca/cacert.pem
 *   bearssl-0.6/build/brssl ta cacert.pem > tls-roots.c
 *
 * Then put this comment block and the include back, and rename brssl's
 * TAs to lp_tls_tas and its TAs_NUM define to lp_tls_tas_num. Nothing
 * else in the generated output is edited. */
...
const br_x509_trust_anchor lp_tls_tas[121] = {
	{
...
const size_t lp_tls_tas_num = 121;
